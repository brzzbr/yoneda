...
object Yoneda {
  ...
  def fromYo[F[_], A](yo: Yoneda[F, A]): F[A] = ???

  def toYo[F[_], A](fa: F[A])(implicit F: Functor[F]): Yoneda[F, A] = ???

  // Йонеда так же ожидаемо является функтором
  implicit def yoFunctor[F[_]]: Functor[({type YF[A] = Yoneda[F, A]})#YF] = ???
...
object CoYoneda {
  ...
  def fromCoYo[F[_], A](coyo: CoYoneda[F, A])(implicit F: Functor[F]): F[A] = ???

  def toCoYo[F[_], A](fa: F[A]): CoYoneda[F, A] = ???

  // Койонеда так же ожидаемо является функтором
  implicit def coyoFunctor[F[_]]: Functor[({type CF[A] = CoYoneda[F, A]})#CF] = ???
